...
class train(tf.keras.Model):
    ...
    def call(self, Y2):
        ...
        sig_q1 = dq1_dY2*sigma/q1
        sig_q2 = dq2_dY2*sigma/q2
        ...
        for n in range(0, int(period)):
            ...
            with tf.GradientTape(persistent=True) as tape:
                tape.watch(Y2)
                q1_tilde, q2_tilde = self.net(Y2)
            dq1_dY2 = tape.gradient(q1_tilde, Y2)
            dq2_dY2 = tape.gradient(q2_tilde, Y2)
            del tape

            sig_q1 = dq1_dY2*sigma/q1_tilde
            sig_q2 = dq2_dY2*sigma/q2_tilde
...
class train_solver(tf.Module):  # network training
    ...
    def __init__(self, vecpar, initial_state, shared_layers):
        ...
        self.model.net.load_weights('./ckpt/Tree_AD_init/my_checkpoint')
        ...
        for it in range(epoch):
            ...
            if (it+1) % 10 == 0:
                ...
                if train_loss <= train_loss_threshold:
                    self.model.net.save_weights('./ckpt/Tree_AD_minloss/my_checkpoint')
                    train_loss_threshold = train_loss

                self.model.net.save_weights('./ckpt/Tree_AD/my_checkpoint')
    ...
    def plot_and_save(self):
        ...
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(Y)
            q1, q2 = self.model.net(Y)
        dq1_dY2 = tape.gradient(q1, Y)
        dq2_dY2 = tape.gradient(q2, Y)
        del tape
        ...
        plt.tight_layout()
        plt.savefig('BSDE_AD.png', dpi=300, bbox_inches='tight')
        #plt.show()
        plt.close(fig)
        ...
        df_results.to_csv('BSDE_AD.csv', index=False)
