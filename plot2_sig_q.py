...
class NeuralNetwork(tf.keras.Model):
    def __init__(self, shared_layers):
        ...
        self.sig_q1_Z1_layers = tf.keras.layers.Dense(32,
                                                   kernel_initializer=initializer,
                                                   bias_initializer='zeros',
                                                   activation=None)

        self.sig_q1_Z2_layers = tf.keras.layers.Dense(32,
                                                   kernel_initializer=initializer,
                                                   bias_initializer='zeros',
                                                   activation=None)

        self.sig_q2_Z1_layers = tf.keras.layers.Dense(32,
                                                   kernel_initializer=initializer,
                                                   bias_initializer='zeros',
                                                   activation=None)

        self.sig_q2_Z2_layers = tf.keras.layers.Dense(32,
                                                   kernel_initializer=initializer,
                                                   bias_initializer='zeros',
                                                   activation=None)

        self.sig_q3_Z1_layers = tf.keras.layers.Dense(32,
                                                   kernel_initializer=initializer,
                                                   bias_initializer='zeros',
                                                   activation=None)

        self.sig_q3_Z2_layers = tf.keras.layers.Dense(32,
                                                   kernel_initializer=initializer,
                                                   bias_initializer='zeros',
                                                   activation=None)
        ...
        self.sig_q1_Z1_net = tf.keras.layers.Dense(1, kernel_initializer=initializer, bias_initializer='zeros', activation=None)
        self.sig_q1_Z2_net = tf.keras.layers.Dense(1, kernel_initializer=initializer, bias_initializer='zeros', activation=None)
        self.sig_q2_Z1_net = tf.keras.layers.Dense(1, kernel_initializer=initializer, bias_initializer='zeros', activation=None)
        self.sig_q2_Z2_net = tf.keras.layers.Dense(1, kernel_initializer=initializer, bias_initializer='zeros', activation=None)
        self.sig_q3_Z1_net = tf.keras.layers.Dense(1, kernel_initializer=initializer, bias_initializer='zeros', activation=None)
        self.sig_q3_Z2_net = tf.keras.layers.Dense(1, kernel_initializer=initializer, bias_initializer='zeros', activation=None)
    ...
    def call(self, Y2, Y3):
        ...
        sig_q1_Z1 = self.sig_q1_Z1_layers(x)
        sig_q1_Z1 = tf.nn.tanh(sig_q1_Z1)
        sig_q1_Z1 = self.sig_q1_Z1_net(sig_q1_Z1)

        sig_q1_Z2 = self.sig_q1_Z2_layers(x)
        sig_q1_Z2 = tf.nn.tanh(sig_q1_Z2)
        sig_q1_Z2 = self.sig_q1_Z2_net(sig_q1_Z2)

        sig_q2_Z1 = self.sig_q2_Z1_layers(x)
        sig_q2_Z1 = tf.nn.tanh(sig_q2_Z1)
        sig_q2_Z1 = self.sig_q2_Z1_net(sig_q2_Z1)

        sig_q2_Z2 = self.sig_q2_Z2_layers(x)
        sig_q2_Z2 = tf.nn.tanh(sig_q2_Z2)
        sig_q2_Z2 = self.sig_q2_Z2_net(sig_q2_Z2)

        sig_q3_Z1 = self.sig_q3_Z1_layers(x)
        sig_q3_Z1 = tf.nn.tanh(sig_q3_Z1)
        sig_q3_Z1 = self.sig_q3_Z1_net(sig_q3_Z1)

        sig_q3_Z2 = self.sig_q3_Z2_layers(x)
        sig_q3_Z2 = tf.nn.tanh(sig_q3_Z2)
        sig_q3_Z2 = self.sig_q3_Z2_net(sig_q3_Z2)

        return q1, q2, q3, sig_q1_Z1, sig_q1_Z2, sig_q2_Z1, sig_q2_Z2, sig_q3_Z1, sig_q3_Z2

shared_layers = [64]
net = NeuralNetwork(shared_layers)
net.load_weights('./ckpt/Tree2_sig_q_minloss/my_checkpoint')
...
# 创建测试数据网格
y2_range = np.linspace(0.2, 4, 100)
y3_range = np.linspace(0.2, 4, 100)
...
q1, q2, q3, sig_q1_Z1, sig_q1_Z2, sig_q2_Z1, sig_q2_Z2, sig_q3_Z1, sig_q3_Z2 = net(Y2, Y3)
...
plt.tight_layout()
plt.savefig('BSDE_3Tree_sig_q.png', dpi=300, bbox_inches='tight')
plt.show()


V1 = q1**2*( sig_q1_Z1**2 + sig_q1_Z2**2)
V2 = q2**2*( sig_q2_Z1**2 + sig_q2_Z2**2)
V3 = q3**2*( sig_q3_Z1**2 + sig_q3_Z2**2)
VC = sig_c_Z1**2 + sig_c_Z2**2


V1_grid = V1.numpy().reshape(100, 100)
V2_grid = V2.numpy().reshape(100, 100)
V3_grid = V3.numpy().reshape(100, 100)
VC_grid = VC.numpy().reshape(100, 100)


fig, axes = plt.subplots(2, 2, figsize=(14, 12))
V1_min, V1_max = np.min(V1_grid), np.max(V1_grid)
im1 = axes[0, 0].imshow(V1_grid,
                       extent=[0.2, 4.0, 0.2, 4.0],
                       origin='lower',
                       aspect='auto',
                       cmap=blue_to_red,
                       vmin=V1_min, vmax=V1_max)  # 固定色彩范围
axes[0, 0].set_xlabel('Y2', fontsize=12)
axes[0, 0].set_ylabel('Y3', fontsize=12)
axes[0, 0].set_title('volatility of q1 ', fontsize=14, fontweight='bold')
plt.colorbar(im1, ax=axes[0, 0], shrink=0.8)
axes[0, 0].grid(True, alpha=0.3, linestyle='--')


V2_min, V2_max = np.min(V2_grid), np.max(V2_grid)
im2 = axes[0, 1].imshow(V2_grid,
                       extent=[0.2, 4.0, 0.2, 4.0],
                       origin='lower',
                       aspect='auto',
                       cmap=blue_to_red,
                       vmin=V2_min, vmax=V2_max)  # 固定色彩范围
axes[0, 1].set_xlabel('Y2', fontsize=12)
axes[0, 1].set_ylabel('Y3', fontsize=12)
axes[0, 1].set_title('volatility of q2', fontsize=14, fontweight='bold')
plt.colorbar(im2, ax=axes[0, 1], shrink=0.8)
axes[0, 1].grid(True, alpha=0.3, linestyle='--')


V3_min, V3_max = np.min(V3_grid), np.max(V3_grid)
im3 = axes[1, 0].imshow(V3_grid,
                       extent=[0.2, 4.0, 0.2, 4.0],
                       origin='lower',
                       aspect='auto',
                       cmap=blue_to_red,
                       vmin=V3_min, vmax=V3_max)  # 固定色彩范围
axes[1, 0].set_xlabel('Y2', fontsize=12)
axes[1, 0].set_ylabel('Y3', fontsize=12)
axes[1, 0].set_title('volatility of q3', fontsize=14, fontweight='bold')
plt.colorbar(im3, ax=axes[1, 0], shrink=0.8)
axes[1, 0].grid(True, alpha=0.3, linestyle='--')


VC_min, VC_max = np.min(VC_grid), np.max(VC_grid)
im4 = axes[1, 1].imshow(VC_grid,
                       extent=[0.2, 4.0, 0.2, 4.0],
                       origin='lower',
                       aspect='auto',
                       cmap=blue_to_red,
                       vmin=VC_min, vmax=VC_max)
axes[1, 1].set_xlabel('Y2', fontsize=12)
axes[1, 1].set_ylabel('Y3', fontsize=12)
axes[1, 1].set_title('volatility of consumption', fontsize=14, fontweight='bold')
plt.colorbar(im4, ax=axes[1, 1], shrink=0.8)
axes[1, 1].grid(True, alpha=0.3, linestyle='--')

plt.tight_layout()
plt.savefig('BSDE_3Tree_vol_sig_q.png', dpi=300, bbox_inches='tight')
plt.show()