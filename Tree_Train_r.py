...
class NeuralNetwork(tf.keras.Model):
    def __init__(self, shared_layers):
        ...
        self.r_layers = tf.keras.layers.Dense(32,
                                          kernel_initializer=initializer,
                                          bias_initializer='zeros',
                                          activation=None)
        ...
        self.r_net = tf.keras.layers.Dense(1, kernel_initializer=initializer, bias_initializer='zeros',
                                            activation=None)
...
class train(tf.keras.Model):
    ...
    def call(self, Y2):
        ...
        q1, q2, sig_q1, sig_q2, r = self.net(Y2)
        ...
        for n in range(0, int(period)):
            ...
            q1_tilde, q2_tilde, sig_q1, sig_q2, r = self.net(Y2)
...
class train_solver(tf.Module):  # network training
    ...
    def __init__(self, vecpar, initial_state, shared_layers):
        ...
        self.model.net.load_weights('./ckpt/Tree_r_init/my_checkpoint')
        ...
        for it in range(epoch):
            ...
            if (it+1) % 10 == 0:
                ...
                if train_loss <= train_loss_threshold:
                    self.model.net.save_weights('./ckpt/Tree_r_minloss/my_checkpoint')
                    train_loss_threshold = train_loss

                self.model.net.save_weights('./ckpt/Tree_r/my_checkpoint')
    ...
    def plot_and_save(self):
        ...
        q1, q2, sig_q1, sig_q2, r = self.model.net(Y)
        ...
        plt.tight_layout()
        plt.savefig('BSDE_r.png', dpi=300, bbox_inches='tight')
        plt.close(fig)
        ...
        df_results.to_csv('BSDE_r.csv', index=False)
