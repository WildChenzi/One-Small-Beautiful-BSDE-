...
class initialize_network(tf.keras.Model):
    ...
    def call(self, Y):
        ...
        q1, q2, sig_q1, sig_q2, r = self.net(Y)
    ...
    def train(self, Y_init, epoch, learning_rate, batch_size):
        ...
        for it in range(epoch):
            ...
            if (it + 1) % 1 == 0:  # Log for each iteration
                ...
                start_time = time.time()
                self.model.net.save_weights('./ckpt/Tree_r_init/my_checkpoint')

shared_layers = [32]
initialize = initialize_network_solver(shared_layers)
initialize.train(Y_init, epoch=10, learning_rate=1e-3, batch_size=512)
